...
interface ScenarioDetection {
  detected_scenario: string;  // "A", "B1", or "B2"
  unit_count: number;
...
function CloudpathImport() {
  ...
  // Scenario Detection (for unit count / coverage info)
  const [scenarioDetection, setScenarioDetection] = useState<ScenarioDetection | null>(null);
  ...
  const analyzeCloudpathData = (data: Record<string, any>): ScenarioDetection => {
    ...
    for (const dpsk of dpsks) {
      ...
      // Detect suffix patterns in username (e.g., "12345_fast" -> "fast")
      const name = dpsk.name || '';
      if (name.includes('_')) {
    ...
    // Always include "gigabit" as default if any users don't have a suffix
    if (usersWithoutSuffix > 0) {
      suffixPatterns.add('gigabit');
    }

    // Update detected suffixes state
    setDetectedSuffixes({
      patterns: Array.from(suffixPatterns),
      withSuffix: usersWithSuffix,
      withoutSuffix: usersWithoutSuffix,
    });
  ...
  const parseApAssignmentCsv = (text: string): {unit_number: string, ap_identifier: string}[] | null => {
    if (!text || text.trim().length === 0) {
      setApAssignmentError("Input appears to be empty");
      setApAssignments([]);
      return null;
    }
    ...
    try {
      ...
      const assignments: {unit_number: string, ap_identifier: string}[] = [];
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        if (i === 0 && (parts[0].toLowerCase() === 'unit_number' || parts[0].toLowerCase() === 'unit')) {
          continue;
        }

        const unit_number = parts[0];
        const ap_identifier = parts[1];

        if (unit_number && ap_identifier) {
          assignments.push({ unit_number, ap_identifier });
        }
      }

      if (assignments.length === 0) {
        setApAssignmentError("No valid AP assignments found. Expected format: unit_number,ap_identifier");
        setApAssignments([]);
        return null;
      }

      setApAssignmentError("");
      setApAssignments(assignments);
      setApAssignmentMode("csv");

      // Count unique units
      const uniqueUnits = new Set(assignments.map(a => a.unit_number));
      console.log(`Loaded ${assignments.length} AP assignments for ${uniqueUnits.size} units`);

      return assignments;
    } catch (err: any) {
      console.error("CSV parse error:", err);
      setApAssignmentError(`Invalid CSV: ${err.message || "Please check the format."}`);
      setApAssignments([]);
      return null;
    }
  };

  // Handle file upload for AP assignments
  const handleApAssignmentUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      ...
      parseApAssignmentCsv(text);
      setApAssignmentText(text || "");
    };

    reader.onerror = () => {
      setApAssignmentError("Failed to read file");
      setApAssignments([]);
    };
  ...
  return (
    <div className="p-4 max-w-6xl mx-auto">
      <h2 className="text-3xl font-bold mb-4">Cloudpath Import</h2>
        ...
        <div className="space-y-4">
          ...
          {scenarioDetection && (
            <div>
              ...
              <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg">
                <div className="text-sm font-medium text-blue-900 mb-1">Import Data Analysis</div>
          ...
          <div className="pt-4 border-t">
            ...
            <input
              ...
              disabled={processing}
              placeholder="e.g., Cloudpath Import"
          ...
          <div className="pt-4 border-t">
            ...
            <input
              ...
              disabled={processing}
              placeholder="e.g., MyPoolOne DPSKs"
              className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 disabled:bg-gray-100"
            />
            <p className="mt-1 text-xs text-gray-500">
            ...
            <div className="space-y-2">
              ...
              <label className={`flex items-start gap-3 p-3 border rounded-lg cursor-pointer transition-colors ${
                ...
                <div className="flex-1">
                  <span className="text-sm font-medium text-gray-900">Skip SSID Configuration</span>
                  <p className="text-xs text-gray-500 mt-1">
                    Import passphrases only. Use when SSIDs already exist or will be configured separately.
                  </p>
                </div>
              </label>

              {/* Option 2: Single Property-Wide SSID */}
              <label className={`flex items-start gap-3 p-3 border rounded-lg cursor-pointer transition-colors ${
                ssidMode === "single"
                  ? "border-blue-500 bg-blue-50"
                  : "hover:bg-gray-50"
                ...
                <input
                  type="radio"
                  checked={ssidMode === "single"}
                  onChange={() => setSsidMode("single")}
                  disabled={processing}
                  className="w-4 h-4 text-blue-600 focus:ring-blue-500 mt-0.5"
                />
                <div className="flex-1">
                  <span className="text-sm font-medium text-gray-900">Single Property-Wide SSID</span>
                  <p className="text-xs text-gray-500 mt-1">
                    Create 1 new DPSK-enabled SSID for all passphrases. All APs broadcast the same SSID.
                  </p>
                </div>
              </label>
              ...
              <label className={`flex items-start gap-3 p-3 border rounded-lg cursor-pointer transition-colors ${
                ssidMode === "per_unit"
                  ? "border-blue-500 bg-blue-50"
                  : "hover:bg-gray-50"
              } ${!scenarioDetection?.can_use_b1 ? "opacity-50" : ""}`}>
                <input
                  type="radio"
                  checked={ssidMode === "per_unit"}
                  onChange={() => setSsidMode("per_unit")}
                  disabled={processing || !scenarioDetection?.can_use_b1}
                  className="w-4 h-4 text-blue-600 focus:ring-blue-500 mt-0.5"
                />
                <div className="flex-1">
              ...
              {ssidMode === "per_unit" && (
                <div className="ml-7 mt-3 space-y-4 p-4 bg-gray-50 rounded-lg border border-gray-200">
                  ...
                  <div>
                    ...
                    {/* File Upload */}
                    <div className="mb-3">
                      <span className="text-xs text-gray-600 font-medium">Upload CSV file:</span>
                      <input
                        type="file"
                        accept=".csv,.txt"
                        onChange={handleApAssignmentUpload}
                        disabled={processing}
                    ...
                    <div>
                      <textarea
                        value={apAssignmentText}
                        onChange={handleApAssignmentTextChange}
                        placeholder="unit_number,ap_identifier
108,R350-ABC123
109,R350-DEF456
110,R550-GHI789"
                        disabled={processing}
                        ...
                        className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm
                          focus:ring-blue-500 focus:border-blue-500 text-sm font-mono
                          placeholder:text-gray-400 disabled:opacity-50 disabled:bg-gray-100"
                      />
                    </div>
                    ...
                    {apAssignments.length > 0 && (
                      <div className="mt-2 p-2 bg-green-50 border border-green-200 rounded text-green-800 text-xs">
                        Loaded {apAssignments.length} AP assignments for {new Set(apAssignments.map(a => a.unit_number)).size} units
                        <button
                          onClick={() => {
                            setApAssignments([]);
                            setApAssignmentMode("skip");
                    ...
                    {apAssignments.length === 0 && !apAssignmentText && (
                      <p className="text-xs text-yellow-600 mt-2">
                        ...
                        You can assign APs manually later.
                      </p>
                    )}
                  </div>
                </div>
          ...
          <div className="space-y-3 pt-4 border-t">
            <label className="flex items-start gap-3">
              <input
                type="checkbox"
                checked={enableAccessPolicies}
                onChange={(e) => setEnableAccessPolicies(e.target.checked)}
                disabled={processing}
                className="w-4 h-4 text-blue-600 rounded focus:ring-blue-500 mt-0.5"
              />
              <div className="flex-1">
                <span className="text-sm font-medium text-gray-900">Enable Access Policies</span>
                <p className="text-xs text-gray-500">
                  Create adaptive policies for rate limiting based on username suffix patterns (e.g., _fast, _gigabit)
                </p>

                {/* Show detected suffixes */}
                {detectedSuffixes && detectedSuffixes.patterns.length > 0 && (
                  <div className="mt-2 p-2 bg-blue-50 border border-blue-200 rounded text-xs">
                    <div className="text-blue-800">
                      <strong>Detected suffixes:</strong> {detectedSuffixes.patterns.join(', ')}
                    </div>
                    <div className="text-blue-600 mt-1">
                      {detectedSuffixes.withSuffix} users with explicit suffix,{' '}
                      {detectedSuffixes.withoutSuffix} defaulting to "gigabit"
                    </div>
                  </div>
                )}
              </div>
            ...
            {/* Policy Set Name - only show when access policies enabled */}
            {enableAccessPolicies && (
              <div className="ml-7">
                <label className="block text-sm font-medium text-gray-700 mb-1">
                  Policy Set Name
                  {poolMetadata?.displayName && policySetName === poolMetadata.displayName && (
                    <span className="ml-2 text-xs font-normal text-blue-600">(auto-filled from pool)</span>
                  )}
                </label>
                <input
                  ...
                  disabled={processing}
                  placeholder={venueName || "Adaptive Policies"}
                ...
                />
                <p className="mt-1 text-xs text-gray-500">
                  Optional: Name for the policy set (defaults to venue name or "Adaptive Policies")
                </p>
              </div>
            )}
      ...
      {v2JobId && (
        <V2PlanConfirmModal
          ...
          workflowName="cloudpath_import"
          apiPrefix="/cloudpath-import/v2"
...
export default CloudpathImport;
