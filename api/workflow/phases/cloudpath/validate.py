...
from workflow.phases.dpsk_usernames import (
    split_account_suffix, is_r1_autogenerated_name,
)
...
class ValidateCloudpathPhase(PhaseExecutor):
    ...
    async def execute(self, inputs: 'Inputs') -> 'Outputs':
        ...
        # Identities RuckusONE named for itself. Never matched to a file entry
        # while a real name exists, and reported so they can be cleaned up.
        autonamed_identities: List[str] = []
                ...
                while True:
                    ...
                    for identity in id_items:
                        ...
                        if desc:
                            ...
                            # An identity R1 named for itself is debris: it
                            # exists because a passphrase create could not
                            # honour our identityId and R1 invented a name.
                            # A properly named twin almost always exists
                            # alongside it carrying the same GUID, and
                            # matching the debris is what makes the next run
                            # attach ANOTHER passphrase to the wrong one --
                            # so the real name always wins, regardless of
                            # which was seen first.
                            if prior and is_r1_autogenerated_name(uname):
                                duplicate_identities.setdefault(
                                    desc, [prior.get('name', '')]
                                ).append(uname)
                                autonamed_identities.append(uname)
                                continue
                            if prior and is_r1_autogenerated_name(prior.get('name')):
                                duplicate_identities.setdefault(
                                    desc, [prior.get('name', '')]
                                ).append(uname)
                                autonamed_identities.append(prior.get('name', ''))
        ...
        for pp in passphrases:
            ...
            # A file entry never legitimately resolves to a name R1 invented.
            if identity_info and is_r1_autogenerated_name(identity_info.get('name')):
                identity_info = None
            if not identity_info:
                # ---------------------------------------------------------
        ...
        if autonamed_identities:
            sample = ", ".join(sorted(set(autonamed_identities))[:5])
            await self.emit(
                f"{len(set(autonamed_identities))} identity(s) in this group "
                f"were named by RuckusONE, not by this import "
                f"({sample}{'...' if len(set(autonamed_identities)) > 5 else ''}). "
                f"They are duplicates of correctly named identities carrying "
                f"the same Cloudpath GUID, left behind when a passphrase "
                f"create could not attach to the real one. This run will use "
                f"the real names and will not add to them; the debris is "
                f"safe to delete once you have checked it.",
                "warning",
            )
