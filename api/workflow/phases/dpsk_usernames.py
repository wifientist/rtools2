...
# RuckusONE names an identity itself when a passphrase is created without a
# username: "DPSK_User_2Uj85Sj1xH_1". Such a name carries no information --
# it is not the resident's account, and nothing downstream can match on it.
#
# It is produced by our own create_passphrase sending `identityId` and NO
# `username`, in the window where R1 could not honour that id (an identity
# group with no DPSK pool attached). R1 minted a persona anyway, and with no
# name supplied it invented one. Verified on a live tenant: 93 Cloudpath
# GUIDs each carried by TWO identities, one properly named and one of these.
R1_AUTO_NAME_PREFIX = "DPSK_User_"


def is_r1_autogenerated_name(name: str) -> bool:
    """
    True if RuckusONE invented this name rather than us supplying one.

    Never treat such an identity as authoritative: do not match a file entry
    to it while a properly named twin exists, and never let a passphrase
    inherit it. Cloudpath usernames are account identifiers, so there is no
    real username this can collide with.
    """
    return str(name or "").startswith(R1_AUTO_NAME_PREFIX)
