...
async def get_msp_labels(r1_client: R1Client = Depends(get_r1_client)):
    return await r1_client.msp.get_msp_labels()

@router.get("/mspEcs")
async def get_msp_ecs(r1_client: R1Client = Depends(get_r1_client)):
    return await r1_client.msp.get_msp_ecs() 

@router.get("/mspTechPartners")
async def get_msp_tech_partners(r1_client: R1Client = Depends(get_r1_client)):
